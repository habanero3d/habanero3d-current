...
namespace Habanero
{
	GenericGeometryBase::GenericGeometryBase(const GenericGeometryBase::InitInfo &initInfo, const void * a_vertexData, size_t a_vertexDataSize, const uint a_numVertices) :
		vertexDataSize(a_vertexDataSize),
		numSubMeshes(initInfo.subMeshes.size()),
		numVertices(a_numVertices)
	{	
		this->vertexData = new char[vertexDataSize];
		memcpy(this->vertexData, a_vertexData, vertexDataSize);
	...
	uint GenericGeometryBase::getNumVertices() const
	{
		return numVertices;
	}
	...
	uint GenericGeometryBase::getNumIndicesSum() const
	{
		uint res = 0u;
		for(uint i = 0 ; i < numSubMeshes; ++i)
			res += subMeshes[i]->getNumIndices();
		return res;
	}
	
	size_t GenericGeometryBase::getIndicesSize() const
	{
		return sizeof(uint) * getNumIndicesSum();
	}

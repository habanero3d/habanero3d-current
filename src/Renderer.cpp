...
namespace Habanero
{
	...
	// może by tak zmieniać ShaderProgramy tylko gdy następny sub-mesh naprawdę korzysta z innego?
		...
		for (uint mesh_no = 0; mesh_no < task->getNumMeshes(); ++mesh_no)
		{
			...
			for (uint subMeshNo = 0; subMeshNo < geometry->getNumSubMeshes(); subMeshNo++)
			{
				...
				if (use_materials)
				{
					...
					shader->setModelMatrix(task->transformation);
					LightSetup.bindShadowMaps(shader);
				}
				else
				{
	...
	void Renderer::drawLines(LinesDrawTask* task)
	{
		...
		glColor4fv((GLfloat*) & Color::Black);
...
Maximum number of texture image units is %d\n\
Maximun number of texture coords is %d",
	...
	void Renderer::executeMemoryOrders(const std::vector<MemorySettingOrder>& orders)
	{
		...
		{
			setMeshCache(orders[i].what, orders[i].what->isSkinned(), orders[i].shouldBeCached);
		}
	}
